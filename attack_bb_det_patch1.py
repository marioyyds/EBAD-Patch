...
def patch_mask_generation(patch=None, image_size=(3, 224, 224), bounding_boxes = None):
    applied_patch = torch.from_numpy(np.zeros(image_size))
    applied_patch_loc = []
...
def PM_tensor_weight_balancing(im, adv, target, w, ensemble, eps, n_iters, alpha, dataset='voc', weight_balancing=False, patch = None):
    ...
    applied_patch = applied_patch.unsqueeze(0).float()
    patch_mask = patch_mask.transpose(1, 2, 0)
    mask_im_np = im_np.copy()
    mask_im_np = mask_im_np * patch_mask
    for i in range(n_iters):
        applied_patch = applied_patch.squeeze(0)
        for loc in applied_patch_loc:
            x, y = loc
            applied_patch[:, y:y + patch.shape[2], x:x + patch.shape[1]] = patch
        applied_patch = applied_patch.unsqueeze(0).float()
        ...
        with torch.no_grad():
            patch = patch - 1*torch.sign(patch.grad)
            patch = patch.clamp(min=-eps, max=eps)
    

    applied_patch = applied_patch.squeeze().detach().cpu().numpy()
